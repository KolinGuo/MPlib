...
void build_pyfcl(py::module &m_all) {
  ...
  /*
  PyTransform3 = py::class_<Transform3, std::shared_ptr<Transform3>>(n,
  "Transform3"); PyTransform3.def() .def()
*/
  ...
  PyConvex
      ...
      .def(py::init([](Matrixx3 const &vertices, Matrixx3I const &faces,
  ...
  PyCollisionObject
      .def(py::init([](const std::shared_ptr<CollisionGeometry> &a,
                       const Vector3 &p, const Vector4 &q) {
  ...
  PyFCLModel
      ...
      .def("update_collision_objects",
           py::overload_cast<std::vector<Vector7> const &>(
