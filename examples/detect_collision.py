...
class DetectCollisionDemo(DemoSetup):
  ...
  def demo(self):
    ...
    self.print_collisions(self.planner.check_for_self_collision(self_collision_free_qpos))

    print("\n----- self-collision pose -----")
    self_collision_qpos = [0, 1.36, 0, -3, -3, 3, -1]
    self.print_collisions(self.planner.check_for_self_collision(self_collision_qpos))
