...
class DetectCollisionDemo(DemoSetup):
    ...
    def print_collisions(self, collisions):
        for collision in collisions:
            print(
                f"{collision.link_name1} of object {collision.object_name1} collides"
                f" with {collision.link_name2} of object {collision.object_name2}"
            )
    ...
    def demo(self):
        ...
        floor = trimesh.creation.box(
            [2, 2, 0.1]
        )  # create a 2x2m floor with thickness 0.1m
        point_cloud, _ = trimesh.sample.sample_surface(floor, 10000)
        point_cloud += [
            0,
            0,
            -0.1,
        ]  # shift the entire point cloud down by 0.1m so we do not collide with the base
        ...
        env_collision_qpos = [
            ...
            0,
            0,
        ]  # this pose causes several joints to dip below the floor
