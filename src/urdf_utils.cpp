...
#define DEFINE_TEMPLATE_URDF_UTILS(DATATYPE)                              \
  ...
  template pinocchio::SE3Tpl<DATATYPE, 0> pose_to_se3<DATATYPE>(          \
      const urdf::Pose &M);                                               \
  template Eigen::Transform<DATATYPE, 3, Eigen::Isometry>                 \
  se3_to_transform<DATATYPE>(const pinocchio::SE3Tpl<DATATYPE, 0> &T);    \
  template Eigen::Transform<DATATYPE, 3, Eigen::Isometry>                 \
  pose_to_transform<DATATYPE>(const urdf::Pose &M);                       \
  ...
  template std::shared_ptr<fcl::Convex<DATATYPE>>                         \
  load_mesh_as_Convex<DATATYPE>(const std::string &mesh_path,             \
                                const Eigen::Matrix<DATATYPE, 3, 1> &scale);
...
template <typename DATATYPE>
Eigen::Transform<DATATYPE, 3, Eigen::Isometry> pose_to_transform(
    const urdf::Pose &M) {
...
template <typename DATATYPE>
pinocchio::InertiaTpl<DATATYPE, 0> convert_inertial(
    const urdf::InertialSharedPtr &Y) {
...
std::shared_ptr<fcl::BVHModel<fcl::OBBRSS<DATATYPE>>> load_mesh_as_BVH(
  ...
  int nbVertices = dfs_build_mesh<DATATYPE>(
      loader.scene, loader.scene->mRootNode, scale, 0, vertices, triangles);
...
std::shared_ptr<fcl::Convex<DATATYPE>> load_mesh_as_Convex(
  ...
  */
  ...
  auto vertices_ptr =
      std::make_shared<std::vector<fcl::Vector3<DATATYPE>>>(vertices);
