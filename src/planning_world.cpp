...
PlanningWorldTpl<DATATYPE>::collideWithOthers(int index,
  ...
  for (size_t i = 0; i < CollisionObjects.size(); i++)
    for (size_t j = 0; j < normal_objects.size(); j++) {
      CollisionResult result;
      result.clear();
      fcl::collide(CollisionObjects[i].get(), normal_objects[j].get(), request,
                   result);
      if (result.isCollision()) {
        WorldCollisionResult tmp;
        tmp.res = result;
  ...
  if (use_point_cloud) {
    if (has_point_cloud == false) {
      ...
      for (size_t i = 0; i < CollisionObjects.size(); i++) {
        CollisionResult result;
        result.clear();
        fcl::collide(CollisionObjects[i].get(), point_cloud.get(), request,
                     result);
        if (result.isCollision()) {
          WorldCollisionResult tmp;
          tmp.res = result;
