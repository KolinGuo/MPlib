...
void PinocchioModelTpl<DATATYPE>::dfs_parse_tree(urdf::LinkConstSharedPtr link,
  ...
  if (verbose) std::cout << link->name << " " << joint.get() << std::endl;
  if (joint) {
  ...
  if (link->child_links.empty()) leaf_links.push_back(link->name);
}
...
void PinocchioModelTpl<DATATYPE>::printFrames(void) {
  std::cout << "Joint dim " << model.joints.size() << " " << model.nv << " "
            << model.nvs.size() << " " << model.idx_vs.size() << std::endl;
  std::cout << "Joint Tangent dim " << model.nq << " " << model.nqs.size()
            << " " << model.idx_qs.size() << std::endl;
  std::cout << "Joint Limit " << model.lowerPositionLimit.size() << " "
            << model.upperPositionLimit.size() << std::endl;
...
std::vector<std::size_t> PinocchioModelTpl<DATATYPE>::getChainJointIndex(
  ...
  std::vector<std::size_t> index_pinocchio =
      model.supports[model.frames[frame_id].parent];
...
Eigen::Matrix<DATATYPE, 7, 1> PinocchioModelTpl<DATATYPE>::getLinkPose(
    size_t const &index) {
  ASSERT(index < link_index_user2pinocchio.size(),
...
Eigen::Matrix<DATATYPE, 7, 1> PinocchioModelTpl<DATATYPE>::getJointPose(
    size_t const &index) {
  ASSERT(index < joint_index_user2pinocchio.size(),
         "The link index is out of bound!");
...
PinocchioModelTpl<DATATYPE>::computeSingleLinkLocalJacobian(
  ...
  J.fill(0);
  pinocchio::computeJointJacobian(model, data, qposUser2Pinocchio(qpos),
                                  jointId, J);
