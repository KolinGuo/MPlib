  ...
  if (link->collision) {
    for (auto geom : link->collision_array) {
      ...
      if (geom_model->type == urdf::Geometry::MESH) {
        ...
        auto mesh_path =
            (boost::filesystem::path(package_dir) / file_name).string();
        if (mesh_path == "") {
          std::stringstream ss;
          ss << "Mesh " << file_name << " could not be found.";
          throw std::invalid_argument(ss.str());
        }
        if (verbose) std::cout << "File name " << file_name << std::endl;
      ...
      collision_origin2link_poses.push_back(
  ...
  for (auto node : pt.get_child("robot")) {
    if (node.first == "disable_collisions") {
      ...
      */
      ...
      for (auto iter = collision_pairs.begin();
           iter != collision_pairs.end();) {
...
void FCLModelTpl<DATATYPE>::printCollisionPairs(void) {
  for (auto cp : collision_pairs) {
    ...
    std::cout << collision_link_names[i] << " " << collision_link_names[j]
              << std::endl;
  }
}
