...
OMPLPlannerTpl<DATATYPE>::random_sample_nearby(VectorX const &start_state) {
  ...
  while (true) {
    ...
    for (int i = 0; i < dim; i++) {
      ...
      new_state[i] +=
          (upper_joint_limits[i] - lower_joint_limits[i]) * ratio * r;
...
OMPLPlannerTpl<DATATYPE>::plan(VectorX const &start_state,
  ...
  for (int i = 0; i < dim; i++) tot_enum_states *= 3;
  ...
  if (solved) {
    ...
    Eigen::Matrix<DATATYPE, Eigen::Dynamic, Eigen::Dynamic> ret(
        len + invalid_start, dim);
    if (verbose) std::cout << "Result size " << len << " " << dim << std::endl;
    if (invalid_start) {
      for (int j = 0; j < dim; j++) ret(0, j) = start_state(j);
    }
    for (size_t i = 0; i < len; i++) {
      ...
      for (size_t j = 0; j < dim; j++) ret(invalid_start + i, j) = res_i[j];
    }
